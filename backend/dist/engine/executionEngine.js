...
class ExecutionEngine {
    ...
    static async run(executionId) {
        let execution = await models_1.Execution.findByPk(executionId);
        if (!execution)
        ...
        try {
            while (execution.status === 'in_progress' && iterations < MAX_ITERATIONS) {
                ...
                const currentStep = await models_1.Step.findByPk(execution.current_step_id, {
                    include: [{ model: models_1.Rule, as: 'rules' }],
                    order: [[{ model: models_1.Rule, as: 'rules' }, 'priority', 'ASC']]
                });
                if (!currentStep) {
                ...
                const rules = currentStep.rules || [];
                try {
                    for (const rule of rules) {
                        const conditionMet = (0, ruleEngine_1.evaluateRule)(rule.condition, execution.data);
                ...
                execution = (await models_1.Execution.findByPk(executionId));
            }
     ...
     */
    static async submitApproval(executionId, stepId, decisionData) {
        const execution = await models_1.Execution.findByPk(executionId);
        if (!execution || execution.status !== 'in_progress')
            return false;
        if (execution.current_step_id !== stepId)
            return false;
