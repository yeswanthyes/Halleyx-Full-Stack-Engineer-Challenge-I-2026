...
router.delete('/', async (req: Request, res: Response) => {
  try {
    ...
    await Execution.destroy({ where: {}, truncate: true });
...
router.get('/:id', async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (execution.current_step_id) {
      current_step = await Step.findByPk(execution.current_step_id);
    }

    const responseData = execution.toJSON();
    (responseData as any).current_step = current_step ? current_step.toJSON() : null;
...
router.post('/workflow/:workflow_id', async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    ExecutionEngine.run(newExecution.id).catch(err => {
...
router.post('/:id/retry', async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    // Start engine asynchronously
    ExecutionEngine.run(execution.id).catch(err => {
...
router.post('/:id/approve/:step_id', async (req: Request, res: Response): Promise<any> => {
  try {
    const { id, step_id } = req.params as { id: string, step_id: string };
    const { decisionData } = req.body;

    const result = await ExecutionEngine.submitApproval(id, step_id, decisionData);
    if (!result) {
